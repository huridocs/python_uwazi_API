...
class Files:
    ...
    def get_document(self, shared_id: str, language: str):
        ...
        file_names_list = list(filter(lambda x: "language" in x and x["language"] == language_document, entity["documents"]))
    ...
    def upload_file(self, pdf_file_path, share_id, language, title):
        try:
            with open(pdf_file_path, "rb") as pdf_file:
                return self.upload_file_from_bytes(pdf_file, share_id, language, title)
    ...
    def delete_file(self, id):
        params = (("_id", id),)
        try:
            self.uwazi_request.request_adapter.delete(
                url=f"{self.uwazi_request.url}/api/files",
                cookies={"connect.sid": self.uwazi_request.connect_sid},
                params=params,
                headers={"X-Requested-With": "XMLHttpRequest"},
            )
