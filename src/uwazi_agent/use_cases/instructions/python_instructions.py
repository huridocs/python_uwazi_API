...
_PYTHON_INSTRUCTIONS_TEMPLATE = Template(
    ...
    "To clear a ``relationship`` (or any multi-valued) property, include it in the "
    "metadata with an empty list ``[]``; omitting a key leaves it unchanged.\n"
    ...
    "write only the shared_id is used. Pass ``[]`` as the value to remove every "
    "related entity for that property.\n"
    "- ``preview``: TEMPLATE-ONLY. ``preview`` tells Uwazi to render the "
    "entity's PRIMARY document (the file the entity is about) as an image at "
    "the top of the entity view — Uwazi auto-generates it from the file, "
    "nothing to upload or set. ``preview`` properties never appear in entity "
    "metadata — there is nothing to read or write. If you see one in the "
    "template, ignore it; do NOT include a ``preview`` key in any update "
    "payload.\n"
    "- ``nested``: TEMPLATE-ONLY parent group. A ``nested`` property gathers "
    "a repeatable sub-set of other properties under one parent key. The "
    "parent itself never appears in entity metadata — only its child "
    "properties do. If you see a ``nested`` parent in the template, ignore "
    "it; do NOT include a ``nested`` key in any update payload (configure "
    "the children individually instead).\n\n"
    ...
    "indent=2)``.\n"
    "- Clear every relationship on a property: "
    "``result = json.dumps(update_entities([{'shared_id': 'abc', 'template_name': "
    "'Books', 'metadata': {'related_authors': []}}]))``.\n"
