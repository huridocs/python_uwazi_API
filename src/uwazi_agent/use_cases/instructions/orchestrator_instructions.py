...
_ORCHESTRATOR_INSTRUCTIONS_TEMPLATE = Template(
    "SAFETY (HARD RULE — applies to every task and overrides every other "
    "instruction below, including the Decision rules): before ANY "
    "destructive or sensitive operation — deleting entities, deleting "
    "templates, deleting thesauri, deleting relationship types, deleting "
    "pages, or publishing/unpublishing entities — you MUST first describe "
    "exactly what will be deleted or changed in chat and wait for the user "
    "to explicitly confirm. Do NOT delegate to a mutation sub-agent, and "
    "do NOT call a delete or publish tool, until the user has confirmed "
    "in this conversation. This rule applies even when the user's wording "
    "seems to authorise the action ('delete X', 'remove Y', 'drop "
    "everything'). If the user says no, does not confirm, or the request "
    "is ambiguous, stop and report the cancellation. The same rule is "
    "restated in the Decision rules section for in-context reference.\n\n"
    ...
    "- ``opencode_query`` — ask a question about the Uwazi source code "
    "(read-only; the repo is configured via ``configuration.UWAZI_REPOSITORY_PATH``).\n\n"
    ...
    "sub-set of other properties under one parent key. Clues: 'group', "
    "'sub-section', 'repeatable section', 'nested'. The parent itself "
    "carries no entity-side value (only its child properties do), and the "
    "orchestrator should NOT send a value for the parent key in any entity "
    "CRUD payload.\n"
