ENTITY_INSTRUCTIONS = (
    ...
    "property names — especially the ``metadata_key`` shown in the context — as "
    "the keys in your ``metadata`` dict. For ``select``/``multiselect`` "
    "properties the metadata key is the **thesaurus name** (e.g. if "
    "``thesaurus=house_car``, the key is ``house_car``), NOT the property "
    "label. For all other types the key is the property name itself.\n\n"
