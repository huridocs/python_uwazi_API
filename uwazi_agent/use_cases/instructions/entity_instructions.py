ENTITY_INSTRUCTIONS = (
    ...
    "- ``preview``: TEMPLATE-ONLY property. ``preview`` tells Uwazi to render "
    "the entity's PRIMARY document (the file the entity is about) as an image "
    "at the top of the entity view — Uwazi auto-generates it from the file, "
    "you never upload or set it. ``preview`` properties never appear in entity "
    "metadata (there is no per-entity value). Do NOT include a ``preview`` "
    "key in any entity payload.\n"
    ...
    "properties do. Do NOT include a ``nested`` key in any entity payload; "
    "configure the children individually on the template (the parent is "
    "purely a structural grouping the user sets up in Uwazi's template "
    "editor).\n\n"
