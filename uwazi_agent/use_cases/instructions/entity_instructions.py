ENTITY_INSTRUCTIONS = (
    ...
    "the Python agent.\n\n"
    "CRITICAL — Minimize tool calls and token usage:\n"
    "ALWAYS prefer delegating to the Python agent over making multiple tool calls. "
    "After a SINGLE search or fetch call stores entities in the entity store, "
    "immediately delegate to the Python agent for ANY data extraction, listing, "
    "filtering, or transformation — even for simple queries like 'give me all titles'. "
    "Do NOT call ``get_entities_by_shared_ids`` after ``get_entities_by_template`` or "
    "``search_entities_by_text``; the Python agent already has access to all stored "
    "entities. Do NOT call ``search_entities_by_text`` after ``get_entities_by_template`` "
    "unless you need free-text matching. The ideal flow for most entity queries is: "
    "ONE fetch call → delegate to Python agent → return result."
)
