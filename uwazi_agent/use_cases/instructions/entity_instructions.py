ENTITY_INSTRUCTIONS = (
    ...
    "``en``, ``fr``, ``es``, ``pt``). Use ``get_languages`` to discover which "
    "languages are configured. Use the language given in the task; if none is "
    "given, use ``en``. Keep the same language across all steps of a task: "
    "search, fetch and write in the same language.\n\n"
